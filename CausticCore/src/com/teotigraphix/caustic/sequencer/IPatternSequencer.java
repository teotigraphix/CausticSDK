...
public interface IPatternSequencer extends IDeviceComponent, IPersist, IRestore
{
    ...
    /**
     * Sets both bank and pattern, signals a phraseChanged() if the phrase
     * exists and has note data.
     * <p>
     * Calling this method on a bank and pattern that has no note data will not
     * automatically create an {@link IPhrase}. It does change the current bank
     * and pattern but the {@link IPhrase} will only be created when
     * {@link #addNote(int, float, float, float, int)} is called.
     * 
     * @param bank The new bank index.
     * @param pattern The new pattern index.
     * @see IPatternSequencerListener#onPhraseChange(IPhrase)
     */
     ...
     */
    IPhrase clearPhrase(int index);

    /**
     * Removes an existing phrase from the sequencer at the bank and pattern.
     * <p>
     * If the phrase does not exist, <code>null</code> will be returned else the
     * existing {@link IPhrase} will be returned.
     * 
     * @param bank The bank index.
     * @param pattern The pattern index.
     */
    IPhrase clearPhrase(int bank, int index);

    /**
     * Returns a boolean indicating if the sequencer contains phrase data at the
     * specified bank and pattern index.
     * 
     * @param bank The bank index.
     * @param pattern The pattern index.
     */
    boolean hasPhrase(int bank, int pattern);

    /**
     * Returns the active phrase based on the {@link #getSelectedBank()} and
    ...
    /**
     * Returns the {@link IPhrase} located at the bank and pattern of the
     * sequencer, <code>null</code> if not found.
     * 
     * @param bank The bank index.
     * @param pattern The pattern index.
     */
    IPhrase getPhraseAt(int bank, int pattern);

    /**
     * Returns all (banks) current {@link IPhrase}s that exist within the
     * machine's sequencer.
     */
    List<IPhrase> getPhrases();

    /**
     * Returns a List of {@link IPhrase}s that exist within the machine's
     * sequencer.
     */
    List<IPhrase> getPhrases(int bank);
    ...
    public interface IPatternSequencerListener
    {
         ...
         * signaling a new phrase is being edited.
         * 
         * @param phrase The new {@link IPhrase}.
         */
        void onPhraseChange(IPhrase phrase, IPhrase oldPhrase);
    }
}
