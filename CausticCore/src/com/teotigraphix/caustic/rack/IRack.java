...
public interface IRack extends IDevice, IPersist
{
    ...
    /**
     * The current {@link IRackSong} for the rack session.
     * 
     * @see #createSong(String, String)
     * @see #loadSong(String)
     * @see #saveSong(String)
     * @see #destroySong()
     */
    ...
    /**
     * Creates a new {@link IRackSong} and initializes a new rack session.
     * <p>
     * A rack song must exist when creating machines. If a song exists, it will
     * be destroyed before the new song is created.
     * <p>
     * If a song already exists when this method is called,
     * {@link #destroySong()} will be called before the new song is created.
     * 
     * @param name The display name of the song.
     * @param path The absolute path to the new song.
     * @throws CausticException
     * @return A new {@link IRackSong} instance.
     */
    IRackSong createSong(String name, String path) throws CausticException;
     ...
     * @return A new {@link IRackSong} instance.
     */
    IRackSong loadSong(String path) throws CausticException;
    ...
    // * @param name The non-unique name of the machine.
    // * @param type The {@link MachineType} to be created.
    // * @param suppress Whether to suppress the core message that creates
    // * a native machine. This is <code>true</code> when loading the rack
    // * from a .caustic file since the native machines are already created.
    // *
    // * @throws CausticException
    // */
    // IMachine addMachineAt(int index, String name, String type, boolean
    // suppress)
    // throws CausticException;
    ...
    public interface OnSongStateChangeListener
    {
         ...
         * 
         * @param song The current {@link IRack} {@link IRackSong} song.
         * @param kind The current state notification.
         */
        void onSongStateChanged(IRackSong song, SongStateChangeKind kind);
    }
