...
public class StepPhraseUtils
{
    ...
    public static void contractResolution(StepPhraseBase phrase,
            Resolution oldValue, Resolution value)
    {
    ...
    public static void expandResolution(StepPhraseBase phrase,
            Resolution oldValue, Resolution value)
    {
        float large = Math.max(oldValue.getValue(), value.getValue());
        float small = Math.min(oldValue.getValue(), value.getValue());
    ...
    public static void updateLength(StepPhraseBase phrase, int oldValue,
            int newValue)
    {
        if (oldValue == -1)
        {
            if (phrase.getStepMap().size() > 0)
                throw new IllegalStateException(
                        "triggerMap already initialized");
     ...
     */
    private static ITrigger createStepTrigger(StepPhraseBase phrase, int index,
            int pitch)
    {
     ...
     */
    private static Trigger initializeStepTrigger(StepPhraseBase phrase,
            int step, int pitch, float gate, float velocity, int flags)
    {
    ...
    private static Trigger initializeNoteTrigger(StepPhraseBase phrase,
            float start, int pitch, float end, float velocity, int flags)
    {
    ...
    @SuppressWarnings("unused")
    private static ITrigger removeBeatTrigger(StepPhraseBase phrase,
            float beat, int pitch)
    {
        int step = Resolution.toStep(beat, phrase.getResolution());
        return removeStepTrigger(phrase, step, pitch);
    }

    private static ITrigger removeStepTrigger(StepPhraseBase phrase, int step,
            int pitch)
    {
