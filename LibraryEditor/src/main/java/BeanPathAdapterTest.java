        ...
        primaryStage.setOnShown(new EventHandler<WindowEvent>() {
            ...
            public void handle(WindowEvent event) {
                ...
                personPA.fieldPathValueProperty().addListener(new ChangeListener<FieldPathValue>() {
                    @Override
                    public void changed(ObservableValue<? extends FieldPathValue> observable,
    ...
    public Parent createRoot() {
        ...
        pBox.valueProperty().addListener(new ChangeListener<String>() {
            ...
            public void changed(ObservableValue<? extends String> observable, String oldValue,
                    String newValue) {
                personPA.setBean(newValue == P1_LABEL ? person1 : newValue == P2_LABEL ? person2
                        : person3);
            }
        });
        pBox.autosize();
        ...
        HBox hobbyBox = beanTF("allHobbies", "hobbies", "name", Hobby.class, 0, ListView.class,
                null, HOBBY_OVERWRITE);
        HBox langBox = beanTF("allLanguages", "languages", null, String.class, 0, ListView.class,
                null, shouldNeverAppear);
        final HBox stBox = beanTF("address.location.state", null, null, null, 2, ComboBox.class,
                "[a-zA-z]", STATES);
        personBox.getChildren()
                .addAll(beanTF("name", null, null, null, 50, null, "[a-zA-z0-9\\s]*"),
                        ...
                        beanTF("password", null, null, null, 100, PasswordField.class,
                                "[a-zA-z0-9]"),
                        beanTF("address.street", null, null, null, 50, null, "[a-zA-z0-9\\s]*"),
                        stBox,
                        beanTF("address.location.country", null, null, null, 10, null, "[0-9]"),
                        beanTF("address.location.country", null, null, null, 2, ComboBox.class,
                                "[0-9]", new Integer[] {
                                        0, 1, 2, 3
                                }),
                        beanTF("address.location.international", null, null, null, 0,
                                CheckBox.class, null),
//                        beanTF("dob", null, null, null, 0, CalendarTextField.class, null),
//                        beanTF("role", null, null, null, 0, ListSpinner.class, null, ROLE1, ROLE2,
                                 langBox, hobbyBox);
        ...
        pojoBox.getChildren()
                .addAll(lbl, new Label("Name:"), pojoNameTF, pojoNameBtn, new Separator(),
                        updateListView(langBox, "Language"), updateListView(hobbyBox, "Hobby"),
                        new Separator(), new Label("POJO Dump:"), pojoTA);
    ...
    public VBox updateListView(HBox langBox, String label) {
        ...
        final ListView<String> listView = (ListView<String>)langBox.getChildren().get(1);
        ...
        remSelBtn.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                Object[] sels = listView.getSelectionModel().getSelectedItems().toArray();
                for (Object sel : sels) {
    ...
    @SafeVarargs
    public final void dumpPojo(final FieldPathValue oldValue, final FieldPathValue newValue,
        ...
        Platform.runLater(new Runnable() {
            ...
            public void run() {
                ...
                for (BeanPathAdapter<Person> p : ps) {
                    dump += "Person {name="
                            ...
                            + (p.getBean().getDob() != null ? new SimpleDateFormat(
                                    ...
                                    .format(p.getBean().getDob().getTime()) : null) + "}\n";
                }
                if (oldValue != null) {
                ...
                System.out.println(String.format("POJO (count=%1$s): %2$s", ++dumpCnt, dump));
                pojoTA.setText(dump);
            }
        });
    }
    ...
    public String dumpHobbyNames(Collection<Hobby> hobbies) {
        ...
        String s = "[";
        for (Hobby h : hobbies) {
            s += '{' + Hobby.class.getSimpleName() + ": name=" + h.getName() + ", desc="
                    + h.getDescription() + ", hashCode=" + h.hashCode() + '}';
        }
        return s += "]";
    }
    ...
    public <T> HBox beanTF(String path, String selectionPath, String itemPath,
            final Class<?> itemType, final int maxChars, Class<? extends Control> controlType,
        ...
        if (controlType == CheckBox.class) {
            ...
            ComboBox<T> cb = new ComboBox<>(FXCollections.observableArrayList(choices));
            ...
            personPA.bindBidirectional(path, cb.valueProperty(), (Class<T>)choices[0].getClass());
            ctrl = cb;
            ...
            // to also pass in the choice class)
            personPA.bindContentBidirectional(path, itemPath, itemType, lv.getItems(),
                    (Class<T>)choices[0].getClass(), null, null);
            if (selectionPath != null && !selectionPath.isEmpty()) {
                ...
                personPA.bindContentBidirectional(selectionPath, itemPath, itemType, lv
                        .getSelectionModel().getSelectedItems(), (Class<T>)choices[0].getClass(),
            ...
            final PasswordField tf = new PasswordField() {
                ...
                private boolean matchTest(String text) {
                    return text.isEmpty()
                            || (text.matches(restictTo) && (getText() == null || getText().length() < maxChars));
                }
            };
            // POJO binding magic...
            ...
            //            personPA.bindBidirectional(path, ctf.calendarProperty(), Calendar.class);
            //            ctrl = ctf;
            //        } else if (controlType == ListSpinner.class) {
            //            final ListSpinner<T> ls = new ListSpinner<>(choices).withCyclic(true);
            //            personPA.bindBidirectional(path, ls.valueProperty(), (Class<T>)choices[0].getClass());
            //            ctrl = ls;
        } else {
        ...
        box.getChildren().addAll(
                new Label(path
                        + (selectionPath != null && !selectionPath.isEmpty() ? " (items) = \n"
                                + selectionPath + " (selected) = " : " = ")), ctrl);
        return box;
    }
    ...
    public static class Hobby {
        ...
        @Override
        public String toString() {
            return String.format("Hobby@%1$s [name=%2$s, description=%3$s]", hashCode(), name,
                    description);
        }
